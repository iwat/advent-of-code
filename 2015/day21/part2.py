...
def worst_fight(boss_hp, boss_dmg, boss_armor):
    worst_lose = None
    ...
    for comb in product(weapons.items(), armors.items(), rings.items(), rings.items()):
        ...
        if winner == 'Boss':
            print('Boss WON !!!', cost, *[name for name, _ in comb])
            if worst_lose is None or cost > worst_lose:
                worst_lose = cost
                worst_combination = tuple([name for name, _ in comb])

    return (worst_combination, worst_lose)
...
if __name__ == '__main__':
    print(worst_fight(boss_hp=109, boss_dmg=8, boss_armor=2))