...
fn part2() -> Result<(), Box<dyn Error>> {
    ...
    let mut last_merge_1: Option<(u32, u32, u32)> = None;
    let mut last_merge_2: Option<(u32, u32, u32)> = None;
    for (key, _distance) in sorted_distance {
        let box1 = (key.0, key.1, key.2);
        let box2 = (key.3, key.4, key.5);
        ...
        match (circuit1, circuit2) {
            (Some(c1), Some(c2)) => {
                if c1 == c2 {
                    ...
                    last_merge_1 = Some(box1);
                    last_merge_2 = Some(box2);
                }
            }
            (Some(c1), None) => {
                println!(
    ...
    println!("Last merge: {:?} -> {:?}", last_merge_1, last_merge_2);
    println!(
        "Answer: {}",
        last_merge_1.unwrap().0 * last_merge_2.unwrap().0
    );
